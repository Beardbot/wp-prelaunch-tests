...
const VERSION     = '0.1.0';
const API_VERSION = 1;
const CAPABILITY  = 'manage_wpt_sensors';

define('WPT_SENSORS_DIR', plugin_dir_path(__FILE__));

// ─── Autoloader ───────────────────────────────────────────────────────────────
// PSR-4 style: WptSensors\Rest\Controller → includes/Rest/Controller.php
...
spl_autoload_register(static function (string $class): void {
    ...
    $path     = WPT_SENSORS_DIR . 'includes/' . str_replace('\\', '/', $relative) . '.php';
    if (file_exists($path)) {
        require $path;
    }
});
...
register_activation_hook(__FILE__, static function (): void {
    if (version_compare(PHP_VERSION, '8.1', '<')) {
        ...
        wp_die(
            'WPT Sensors requires PHP 8.1 or higher. This site runs PHP ' . esc_html(PHP_VERSION) . '.',
    ...
    if (version_compare(get_bloginfo('version'), '6.0', '<')) {
        deactivate_plugins(plugin_basename(__FILE__));
        wp_die(
            'WPT Sensors requires WordPress 6.0 or higher.',
            'Plugin activation failed',
            ['back_link' => true]
        );
    }
    ...
    // this capability and nothing else (`wp user add-cap <user> manage_wpt_sensors`).
...
// ─── REST sensor surface ─────────────────────────────────────────────────────
// Every route in the wpt-sensors/v1 namespace is gated by Rest\Controller::authorize()
// — encrypted transport, WordPress application-password authentication, and the
// manage_wpt_sensors capability. The filters below integrate with core's own
