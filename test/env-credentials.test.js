...
beforeEach(() => {
  ...
  delete process.env.TEST_CRED_NAME_LQS_BEARDBOT_DEV;
});

test('envKeySuffix mangles every non-alphanumeric to underscore and uppercases', () => {
  assert.equal(envKeySuffix('lqs.beardbot.dev'), 'LQS_BEARDBOT_DEV');
...
test('per-site override wins over the global fallback', () => {
  ...
  process.env.TEST_CRED_NAME_LQS_BEARDBOT_DEV = 'site-value';

  const resolved = envForSite('lqs.beardbot.dev', 'TEST_CRED_NAME');
...
test('resolution reports the exact env var names for error messages', () => {
  ...
  assert.equal(resolved.perSiteName, 'TEST_CRED_NAME_LQS_BEARDBOT_DEV');
  assert.equal(resolved.globalName, 'TEST_CRED_NAME');
  assert.equal(resolved.suffix, 'LQS_BEARDBOT_DEV');
});
